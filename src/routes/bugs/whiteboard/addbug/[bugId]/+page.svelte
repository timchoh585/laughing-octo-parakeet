<script>
  ...
  let whiteboard = $page.params.whiteboard;
  ...
  const submitBug = async () => {
    try {
      ...
      setTimeout(() => {
        goto(`/bugs/whiteboard/${encodeURIComponent(whiteboard)}`);
      }, 2000);
    } catch (err) {
...
<div class="container">
  ...
  <div class="form-group">
    ...
    <input type="text" id="assignedTo" bind:value={$assignedTo} placeholder="Enter assignee" />
  </div>

  <div class="form-group">
    <label for="whiteboard">Whiteboard</label>
    <textarea type="text" id="whiteboard" bind:value={$whiteboardField} placeholder="Enter whiteboard" />
  </div>
